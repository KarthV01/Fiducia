...
export class FakePrisma {
  ...
  creatorWalletConnection = {
    ...
    findMany: async ({ where = {}, select }: { where?: any; select?: any } = {}) => this.creatorWalletConnections.filter((item) => (!where.creatorProfileId || (typeof where.creatorProfileId === "string" ? item.creatorProfileId === where.creatorProfileId : where.creatorProfileId.in?.includes(item.creatorProfileId))) && (!where.isPrimary || item.isPrimary) && (!where.verifiedAt?.not || item.verifiedAt !== null) && (where.revokedAt !== null || item.revokedAt === null)).map((item) => select?.address ? { address: item.address } : select?.creatorProfileId ? { creatorProfileId: item.creatorProfileId } : item),
