          ...
          {selected ? <>
            ...
            <div className="shrink-0 border-t border-rule bg-surface p-3">
              ...
              <div onDragOver={(event) => event.preventDefault()} onDrop={(event) => addDroppedFiles(event, setFiles, setError)}>
                <textarea aria-label="Write a message" className="min-h-16 w-full resize-none rounded-lg border border-rule bg-canvas/50 px-3 py-2 text-sm outline-none focus:border-[#a599ef]" rows={2} value={body} onChange={(event) => handleTyping(event.target.value)} onBlur={() => void api.updateConversationState(identityId, selected.id, { draftText: body || null }).catch(() => undefined)} onKeyDown={(event) => { if (event.key === "Enter" && !event.shiftKey && !event.nativeEvent.isComposing) { event.preventDefault(); void send(); } }} placeholder="Write a message…" />
                <div className="mt-2 flex items-center justify-between gap-2">
                  <label className="relative inline-flex h-8 cursor-pointer items-center gap-1.5 rounded-md px-2 text-xs text-muted hover:bg-accent-soft focus-within:outline-2 focus-within:outline-[#b4aaff]" title="Attach files"><span aria-hidden="true" className="text-lg">＋</span> Attach<input aria-label="Attach files" className="sr-only" type="file" multiple accept={ATTACHMENT_ACCEPT} onChange={(event) => { validateFiles(Array.from(event.target.files ?? []), setFiles, setError); event.target.value = ""; }} /></label>
                  <Button type="button" className="h-8 px-4 text-xs" disabled={busy || (!body.trim() && !files.length)} onClick={() => void send()}>Send</Button>
                </div>
              </div>
            </div>
          </> : <div className="m-auto max-w-sm px-8 text-center">{selectedId ? <p className="text-sm text-muted">Opening conversation…</p> : <><span className="mx-auto flex h-14 w-14 items-center justify-center rounded-2xl border border-rule bg-surface text-[#b4aaff]"><Icon name="messages" width="26" height="26" /></span><h2 className="mt-5 text-lg font-semibold">A conversation starts something.</h2><p className="mt-2 text-sm leading-relaxed text-muted">Connect with the people behind your next collaboration. Select a conversation to get started.</p></>}</div>}
        </section>
...
function MessageBubble({ message, own, identityId, onReply, onChanged }: { message: ChatMessage; own: boolean; identityId: string; onReply: () => void; onChanged: () => void }) {
  ...
  return <div className="group flex gap-2.5">
    ...
    <div className="min-w-0 flex-1">
      ...
      <div className={`rounded-lg border px-3 py-2.5 text-sm leading-relaxed break-words ${own ? "border-[#39354d] bg-[#242231]" : "border-rule bg-surface"}`}>
        {message.deletedAt ? <em className="text-muted">Message deleted</em> : <>
          {message.replyTo ? <div className="mb-2 border-l-2 border-[#a599ef]/50 pl-2 text-xs text-muted">{message.replyTo.sender.displayName}: {message.replyTo.body}</div> : null}
          {message.body ? <p className="whitespace-pre-wrap">{message.body}</p> : null}
          {message.attachments.map((attachment) => <a key={attachment.id} className="mt-2 block text-xs text-[#c1b9ff] underline underline-offset-4" href={`/api/profiles/${encodeURIComponent(identityId)}/attachments/${attachment.id}/content`} target="_blank" rel="noreferrer">{attachment.fileName}</a>)}
        </>}
      </div>
      {!message.deletedAt ? <div className="mt-1 flex min-h-5 flex-wrap items-center gap-2 text-[11px]">
        ...
        {message.failed ? <span className="text-[#f49ba5]">Not sent. Please try again.</span> : null}
        {!message.sending && !message.failed ? <div className="message-actions flex items-center gap-1">
          ...
          {own ? <details className="relative"><summary aria-label="Message options" className="list-none rounded px-1.5 text-muted hover:bg-accent-soft">···</summary><div className="absolute right-0 top-6 z-10 flex gap-3 rounded-lg border border-rule bg-surface p-3 shadow-xl">
            <button type="button" className="text-[#c1b9ff]" onClick={() => { const body = window.prompt("Edit message", message.body ?? ""); if (body) void mutate(api.editMessage(identityId, message.conversationId, message.id, body)); }}>Edit</button>
            <button type="button" className="text-[#f49ba5]" onClick={() => { if (window.confirm("Delete this message?")) void mutate(api.deleteMessage(identityId, message.conversationId, message.id)); }}>Delete</button>
          </div></details> : null}
        </div> : null}
      </div> : null}
      {error ? <p role="alert" className="mt-1 text-xs text-[#f49ba5]">{error}</p> : null}
    </div>
  </div>;
}
