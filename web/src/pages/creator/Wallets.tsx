...
import { connectWalletConnect, discoverMetaMask, requestWalletAccount, signWalletMessage, walletErrorMessage, type Eip1193Provider, type WalletClient } from "../../lib/evmProvider";
...
export function CreatorWalletsPage() {
  ...
  async function connect(walletClient: WalletClient) {
    ...
    try {
      const found = walletClient === "walletconnect" ? await connectWalletConnect() : await discoverMetaMask();
      if (!found) { setShowOnboarding(true); return; }
      setProvider(found);
      const account = await requestWalletAccount(found); setActiveAccount(account.address);
      const challenge = await api.creatorWalletChallenge(creatorId, account.address, account.chainId);
      const signature = await signWalletMessage(found, account.address, challenge.message);
      await api.connectCreatorWallet(creatorId, { ...challenge, signature, walletClient });
      reload();
  ...
  return <div className="max-w-5xl">
    <PageHeader title="Wallets" description="Verify the addresses you control and choose where future creator payouts should go." action={<div className="flex flex-wrap gap-2"><Button onClick={() => void connect("metamask")} disabled={Boolean(busy)}><Icon name="wallet" />{busy === "metamask" ? "Check MetaMask..." : "MetaMask extension"}</Button><Button variant="secondary" onClick={() => void connect("walletconnect")} disabled={Boolean(busy)}>{busy === "walletconnect" ? "Opening wallets..." : "Phone or QR"}</Button></div>} />
    ...
    {activeAccount && data?.wallets.some((wallet) => !wallet.revokedAt && wallet.address.toLowerCase() !== activeAccount.toLowerCase()) ? <div className="mb-5"><Banner tone="info">Your connected wallet changed to {shortAddress(activeAccount)}. Connect it separately if you want to add it to this profile.</Banner></div> : null}
...
function walletSourceLabel(source: string) { return source === "metamask" ? "MetaMask" : source === "walletconnect" ? "WalletConnect" : source === "external_verified" ? "a verified wallet" : source; }
