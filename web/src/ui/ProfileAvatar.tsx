...
export function ProfileAvatar({ profile, small = false }: { profile: Pick<SocialProfile, "avatarUrl" | "displayName">; small?: boolean }) {
  return (
    <div className={`flex shrink-0 items-center justify-center overflow-hidden rounded-full border border-[#443e59] bg-accent-soft font-semibold text-[#c1b9ff] ${small ? "h-8 w-8 text-xs" : "h-10 w-10"}`}>
