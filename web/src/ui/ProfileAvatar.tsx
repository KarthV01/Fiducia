...
export function ProfileAvatar({ profile, small = false }: { profile: Pick<SocialProfile, "avatarUrl" | "displayName">; small?: boolean }) {
  return (
    <div className={`flex shrink-0 items-center justify-center overflow-hidden rounded-full bg-accent-soft font-semibold text-accent ${small ? "h-8 w-8 text-xs" : "h-10 w-10"}`}>
