...
export function PublicLayout({ children, signedIn = false }: { children: ReactNode; signedIn?: boolean }) {
  ...
  return <div className="flex min-h-screen flex-col bg-canvas text-ink">
    <header className="border-b border-rule"><div className="mx-auto flex h-20 max-w-6xl items-center justify-between gap-4 px-5 sm:px-8">
      <Link to={signedIn ? "/accounts" : "/"} aria-label="Fiducia home"><BrandMark size="sm" withName /></Link>
    ...
    <footer className="mx-auto flex w-full max-w-6xl flex-wrap justify-between gap-3 border-t border-rule px-5 py-6 text-xs text-muted sm:px-8"><span>Fiducia · A workspace for creator partnerships.</span><span>© {new Date().getFullYear()} Fiducia</span></footer>
  </div>;
}
